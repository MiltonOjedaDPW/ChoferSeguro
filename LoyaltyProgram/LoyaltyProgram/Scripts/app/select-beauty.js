class SelectBeauty {
    ...
    {
        this.el = data.el;
        this.placeholder = (typeof data.placeholder !== "undefined") ? data.placeholder : "Elige una regla";
    ...
    htmlData()
    {
        ...
        for(name in this.tempData) {
            html += `
                <li>
                    <a href="#" data-value="${name}"><i class="fa ${this.tempData[name].icon} fa-3x"></i> ${this.tempData[name].text}</a>
                </li>
            `;
        }
    ...
    getHtml()
    {
        this.htmlBeauty = `<div class="select-beauty" data-instance="${this.el}">
            <button id="txtListas_Faltas" class="button-sb button-sb-block">${this.placeholder}</button>
            <ul class="hide">
                ${this.htmlData()}
            </ul>
        </div>`;
